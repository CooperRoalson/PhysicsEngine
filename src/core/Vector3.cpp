...
Vector3 Vector3::normalized() const {return (isZero()) ? Vector3(0,0,0) : operator/(magnitude());}
...
bool Vector3::isZero() const {return abs(x)<=REAL_ROUNDING_THRESHOLD && abs(y)<=REAL_ROUNDING_THRESHOLD && abs(z)<=REAL_ROUNDING_THRESHOLD;}
